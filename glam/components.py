...
def tt_drift_multiplicative(v, tau, gamma, values, gaze, zerotol):
    """
    theano implementation of the multiplicative GLAM drift
...
def tt_drift_additive(v, tau, gamma, values, gaze, zerotol):
    """
    theano implementation of the additive GLAM drift
    vectorized, i.e., runs on all trials simultaneously
    """

    E_drifts = values + gamma * gaze
    n_items = tt.cast(E_drifts.shape[1], dtype='int32')
    stacked_E = tt.repeat(E_drifts, repeats=n_items, axis=1).T
    stacked_E_reshaped = tt.reshape(stacked_E, newshape=(
        E_drifts.shape[1], E_drifts.shape[1], E_drifts.shape[0])).T
...
def drift(drift='multiplicative'):
    if drift == 'multiplicative':
        return tt_drift_multiplicative
    elif drift == 'additive':
        return tt_drift_additive
    else:
