...
def simulate_subject(parameters, values, gaze, n_repeats=1, subject=0, boundary=1, error_weight=0.05, error_range=(0, 5000), drift='multiplicative'):
...
def drift_multiplicative(v, tau, gamma, values, gaze):
    n_items = len(values)
...
def drift_additive(v, tau, gamma, values, gaze):
    n_items = len(values)

    absolute = values + gamma * gaze
    relative = np.zeros(n_items)
...
def predict(model, n_repeats=1, boundary=1., error_weight=0.05, verbose=True, drift='multiplicative'):
    """
    ...
    for p, parameter in enumerate(['v', 'gamma', 's', 'tau', 't0']):
        prediction[parameter] = parameters[p]
