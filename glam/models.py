...
class GLAM(object):
    ...
    """
    def __init__(self, data=None, drift='multiplicative'):
        self.data = data
    ...
    def make_model(self, kind, depends_on=dict(v=None, gamma=None, s=None, tau=None, t0=None), **kwargs):
        ...
        self.model = make_models(df=self.data, kind=kind, design=self.design, drift=self.drift, **kwargs)
...
def make_models(df, kind, verbose=True, design=dict(v=None, gamma=None, s=None, tau=None, t0=None), drift='multiplicative', **kwargs):
...
def make_subject_model(rts, gaze, values, error_ll,
    ...
    with pm.Model() as glam_individual:
        ...
        v_testval = np.exp(-np.log(rts.mean()) - 2)
        ...
        s = generate_subject_model_parameters(parameter='s',
                                              ...
                                              val=s_val, testval=100*v_testval) #0.0075)

        tau = generate_subject_model_parameters(parameter='tau',
                                                design=design['tau'],
                                                lower=0, upper=5,
                                                val=tau_val, testval=0.75)
        ...
        def lda_logp(rt,
            ...
            drift = ifelse(is_multiplicative,
                           glam.components.tt_drift_multiplicative(v[0, tt.cast(v_index, dtype='int32')][:, None],
                                                                   tau[0, tt.cast(tau_index, dtype='int32')][:, None],
                                                                   gamma[0, tt.cast(gamma_index, dtype='int32')][:, None],
                                                                   values,
                                                                   gaze,
                                                                   zerotol),
                           glam.components.tt_drift_additive(v[0, tt.cast(v_index, dtype='int32')][:, None],
                                                             tau[0, tt.cast(tau_index, dtype='int32')][:, None],
                                                             gamma[0, tt.cast(gamma_index, dtype='int32')][:, None],
                                                             values,
                                                             gaze,
...
def make_hierarchical_model(rts, gaze, values, error_lls,
    ...
    with pm.Model() as glam_hierarchical:
        ...
        def lda_logp(rt,
            ...
            # compute drifts
            drift = ifelse(is_multiplicative,
                           glam.components.tt_drift_multiplicative(v[tt.cast(v_subject_index, dtype='int32'), 
                                                                   ...
                                                                   gaze,
                                                                   zerotol),
                           glam.components.tt_drift_additive(v[tt.cast(v_subject_index, dtype='int32'), 
                                                               tt.cast(v_condition_index, dtype='int32')][:, None],
                                                             tau[tt.cast(tau_subject_index, dtype='int32'),
                                                                 tt.cast(tau_condition_index, dtype='int32')][:, None],
                                                             gamma[tt.cast(gamma_subject_index, dtype='int32'),
